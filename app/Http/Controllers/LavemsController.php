...
public function releaseOfFunds(Request $request)
{
    // Make a GET request to an external API to get invoice data
    $theUrl = config('app.guzzle_test_url').'/api/release-of-funds/'.$request->id;
    $response = Http::get($theUrl);
    ...
    // Generate the PDF receipt using the extracted data
    $pdf = \PDF::loadView('project.release-of-funds', compact('data'), ['invoice_number'=>$invoice_number, 'client_id'=>$client_id, 'contact_address'=>$contact_address, 'nature_of_business'=>$nature_of_business, 'client_name'=>$client_name, 'invoice_date'=>$invoice_date, 'phone_number'=>$phone_number]);
